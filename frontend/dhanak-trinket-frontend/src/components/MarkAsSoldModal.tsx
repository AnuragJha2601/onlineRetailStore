...
import { Product, RecordSaleRequest, SaleType, SaleDto, SALE_CHANNELS } from '@/types/product';
...
export default function MarkAsSoldModal({ product, onClose, onSaleRecorded, onError }: MarkAsSoldModalProps) {
    ...
    const [recordedSale, setRecordedSale] = useState<SaleDto | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.success && result.data) {
                // Show success state inside the modal — don't close yet
                // (lets you inspect the network tab before dismissing)
                setRecordedSale(result.data);
            } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
                    ...
                    {/* Inline success — stays open so you can inspect the network tab */}
                    {recordedSale ? (
                        <div className="space-y-3">
                            <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-4">
                                <p className="text-sm font-semibold text-green-800 mb-2">✓ Sale recorded (ID #{recordedSale.id})</p>
                                <dl className="text-sm text-green-700 space-y-1">
                                    <div className="flex justify-between"><dt>Product</dt><dd className="font-medium">{recordedSale.productName}</dd></div>
                                    <div className="flex justify-between"><dt>Type</dt><dd className="font-medium">{recordedSale.saleType}</dd></div>
                                    <div className="flex justify-between"><dt>Qty × Price</dt><dd className="font-medium">{recordedSale.quantitySold} × ₹{recordedSale.sellingPrice}</dd></div>
                                    <div className="flex justify-between border-t border-green-200 pt-1 mt-1"><dt className="font-semibold">Total</dt><dd className="font-bold">₹{recordedSale.totalAmount.toFixed(0)}</dd></div>
                                </dl>
                            </div>
                            <button
                                type="button"
